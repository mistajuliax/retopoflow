...
class AddonLocator(object):
    def __init__(self, f=None):
        self.fullInitPath = f or __file__
...
def selection_mouse():
    ...
    if select_type == 'RIGHT':
        select_mouse.extend(('RIGHTMOUSE', 'SHIFT+RIGHTMOUSE'))
    else:
        select_mouse.extend(('LEFTMOUSE', 'SHIFT+LEFTMOUSE'))
    return select_mouse
...
class Profiler(object):
    class ProfilerHelper(object):
        def __init__(self, pr, text):
            full_text = (f'{pr.stack[-1].text}^' if pr.stack else '') + text
...
def ray_cast_region2d(region, rv3d, screen_coord, ob, settings):
    ...
    if settings.debug > 3:
        print(f'ray_persp  = {str(rv3d.is_perspective)}')
        print(f'ray_origin = {str(ray_origin)}')
        print(f'ray_target = {str(ray_target)}')
        print(f'ray_vector = {str(ray_vector)}')
        print(f'ray_diff   = {str((ray_target - ray_origin).normalized())}')
        print(f'start:  {str(ray_start_local)}')
        print(f'target: {str(ray_target_local)}')
...
def ray_cast_path(context, ob, screen_coords):
    ...
    return [mx*co for co,no,face in hits if face != -1]
...
def ray_cast_stroke(context, ob, stroke):
    ...
    return [(mx*hit[0],stroke[i][1])  for i,hit in enumerate(hits) if hit[2] != -1]
...
def get_object_length_scale(o):
    ...
    return (min(bbox)-max(bbox)).length
...
def ray_cast_visible(verts, ob, rv3d):
    ...
    if rv3d.is_perspective:
        ...
        source = [eyeloc_local for _ in verts]
    else:
...
def zip_pairs(l):
    yield from zip(l, itertools.chain(l[1:],l[:1]))
