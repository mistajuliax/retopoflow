...
def diagonal_verts(verts):
    ...
    return math.pow((dx**2 + dy**2 + dz**2),.5)
...
def calculate_com_normal(locs):
    ...
    '''
    com = sum(locs, Vector((0,0,0))) / len(locs)
...
def cross_section(bme, mx, point, normal, debug = True):
    ...
    for ed in bme.edges:
        ...
        elif v := intersect_line_plane(A, B, pt, no):
    ...
    return (verts, eds) if len(verts) else None
...
def point_inside_loop2d(loop, point):
    ...
    return bool(math.fmod(intersections,2))
...
def point_inside_loop_almost3D(pt, verts, no, p_pt = None, threshold = .01, debug = False, bbox = False):
    ...
    if not bbox:
        return point_inside_loop2d(verts_prime, pt_prime)

    print('intersected the bbox')
    return point_inside_loop2d(bound_loop, pt_prime)
    ...
    for ed in face.edges:
        if ed.index not in prev_eds:
            ...
            if result[0] == 'CROSS':
                ...
                co_point = ed.verts[0] if result[1] == A else ed.verts[1]
            ...
            if ed.index not in prev_eds:
                ...
                if result[0] == 'CROSS':
                    ...
                    if cop_face == 2:
                        ...
                        element = ed.verts[1] if ed.verts[0].index == vert.index else ed.verts[0]
...
def space_evenly_on_path(verts, edges, segments, shift = 0, debug = False):    #prev deved for Open Dental CAD
    ...
    for i in range(segments- 1 + cyclic * 1):
...
def com_line_cross_test(com1, com2, pt, no, factor = 2):
    ...
    if v := intersect_line_plane(com1, com2, pt, no):
...
def discrete_curl(verts, z):    #Adapted from Open Dental CAD by Patrick Moore
    ...
    closed = verts[-1] == verts[0]
    for n in range(len(verts) - 1*closed):
...
def rot_between_vecs(v1,v2, factor = 1):
    ...
    return Quaternion((cos, sin*axis[0], sin*axis[1], sin*axis[2]))
...
def interp_curve(curve, iterations):
    ...
    for _ in range(iterations):
        newpoints = []
        for i in range(len(new_curve)-3):
...
def intersect_paths(path1, path2, cyclic1 = False, cyclic2 = False, threshold = .00001):
    ...
    for i in range(len(path1) + 1*cyclic1 - 1):
        
        n = int(math.fmod(i+1, len(path1)))
        v1 = path1[n]
        v2 = path1[i]
        for j in range(len(path2) + 1*cyclic2 - 1):
            
            m = int(math.fmod(j+1, len(path2)))
            v3 = path2[m]
            v4 = path2[j]

            if intersect := intersect_line_line(v1, v2, v3, v4):
...
def pole_detector(bme):
    ...
    return [vert.index for vert in bme.verts if len(vert.link_edges) in {3,5,6}]
...
def align_edge_loops(verts_1, verts_2, eds_1, eds_2):
    ...
    shift_lengths = []
    #shift_cross = []
    for shift in range(len(verts_2)):
        tmp_len = 0
        #tmp_cross = 0
        for i in range(len(verts_2)):
...
def cross_section_until_plane(bme, mx, point, normal, seed, pt_stop, normal_stop, max_tests = 10000, debug = True):
    ...
    if len(verts):
        ...
        #loose chains only
        if not plane_chains and len(loose_chains):
    ...
    for initial_element in seeds: #this will go both ways if they dont meet up.
        ...
        while element and total_tests < max_tests and stop_test != seed_index_b:
            ...
            if type(element) == bmesh.types.BMFace:
                ...
                stop_test = element.index if element else None
            elif type(element) == bmesh.types.BMVert:
                print('do we ever use the vert cycle?')
