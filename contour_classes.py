...
class ContourCutSeries(object):  #TODO:  nomenclature consistency. Segment, SegmentCuts, SegmentCutSeries?
    def __init__(self, context, raw_points,
        ...
        self.raw_screen = [
            v for i, v in enumerate(raw_points) if not math.fmod(i, cull_factor)
        ]
    ...
    def smooth_path(self,context, ob = None):
        ...
        if len(self.knots) > 2:
            ...
            segments = [
                [
                    self.raw_world[m]
                    for m in range(self.knots[i], self.knots[i + 1])
                ]
    ...
    def create_cut_nodes(self,context, knots = False):
        ...
        if len(self.knots) > 2 and knots:
            segments = [
                self.world_path[self.knots[i] : self.knots[i + 1] + 1]
                for i in range(len(self.knots) - 1)
            ]
        ...
        for i, segment in enumerate(segments):
            ...
            if i > 0:
                self.cut_points.extend(vs[1:])
            else:
                self.cut_points.extend(vs[:])
    ...
    def backbone_from_cuts(self,context,ob,bme):
        ...
        for i, cut in enumerate(self.cuts):
            ...
            if i == 0:
                ...
                if vertebra := contour_utilities.cross_section_seed_direction(
                    bme,
                    ob.matrix_world,
                    pt,
                    cut_no,
                ...
                if vertebra := contour_utilities.cross_section_seed_direction(
                    ...
                    -1 * v1,
                    stop_plane=[
                        self.cuts[i - 1].plane_com,
                        self.cuts[i - 1].plane_no,
                    ],
                    max_tests=1000,
                )[0]:
                    vertebra3d = [ob.matrix_world * v for v in vertebra]
